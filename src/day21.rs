...
fn part1(program: &Program) -> usize {
	...
	while regs[2] < program.len()  {
		...
		match program[regs[2]].0.as_str() {
			"setr" => regs = setr(&program[regs[2]].1, &regs),
			"eqrr" => regs = eqrr(&program[regs[2]].1, &regs),
			"gtri" => regs = gtri(&program[regs[2]].1, &regs),
			"muli" => regs = muli(&program[regs[2]].1, &regs),
			"eqir" => regs = eqir(&program[regs[2]].1, &regs),
			"borr" => regs = borr(&program[regs[2]].1, &regs),
			"bori" => regs = bori(&program[regs[2]].1, &regs),
			"mulr" => regs = mulr(&program[regs[2]].1, &regs),
			"gtrr" => regs = gtrr(&program[regs[2]].1, &regs),
			"seti" => regs = seti(&program[regs[2]].1, &regs),
			"banr" => regs = banr(&program[regs[2]].1, &regs),
			"eqri" => regs = eqri(&program[regs[2]].1, &regs),
			"addr" => regs = addr(&program[regs[2]].1, &regs),
			"gtir" => regs = gtir(&program[regs[2]].1, &regs),
			"addi" => regs = addi(&program[regs[2]].1, &regs),
			"bani" => regs = bani(&program[regs[2]].1, &regs),
...
pub fn main() {
	let instructions = parse_input("./input/day21/input.txt");
